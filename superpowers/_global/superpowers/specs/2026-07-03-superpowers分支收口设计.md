...
后续实施计划也必须写入 `/mnt/f/IdeaProjects/goto-software/goto-docs/superpowers/_global/superpowers/plans`，不得写入 `/mnt/g/Obsidian/docs/superpowers` 或 `/mnt/g/Obsidian/skill/superpowers/docs/superpowers`。
...
3. **最后做统一验证**
   ...
   - `goto-superpowers-docs` 验证命令必须至少包含 `git -C /mnt/f/IdeaProjects/goto-software/goto-docs status --short --branch`。历史未跟踪文件不整理，但最终报告必须区分“本次文件已提交”和“既有未跟踪文件仍存在”。
